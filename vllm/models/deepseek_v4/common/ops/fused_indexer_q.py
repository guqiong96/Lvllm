...
class FusedIndexerQRopeQuantTritonKernel(
    ...
    def kernel(
        ...
        # (e4m3fn) elsewhere -- matches the K cache. Pre-Ada (SM8x) Triton cannot
        # name fp8e4nv even as a pointer: the launcher passes a uint8 view and
        # the byte is encoded bit-wise (identical to the RNE hardware cast).
        fp8_base_ptr = (
            index_q_fp8_ptr
    ...
    def __call__(
        ...
        *,
        ...
        return (num_tokens, num_index_q_heads), dict(
            ...
            index_q_fp8_ptr=(
                # Warmup tensors are already built with the emulate dtype;
                # runtime fp8 tensors need the explicit uint8 reinterpret.
                index_q_fp8.view(torch.uint8)
                if emulate_fp8 and isinstance(index_q_fp8, torch.Tensor)
                else index_q_fp8
            ),
