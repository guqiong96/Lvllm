...
[`patches/01_lk_moe__71888f507a.patch`](./patches/01_lk_moe__71888f507a.patch) — the full diff between
upstream base commit `71888f507a` and the lk_moe fork. Apply it to
a clean `71888f507a` checkout with `git apply patches/01_lk_moe__71888f507a.patch`.
...
```bash
2026-09-10: Lvllm - rebuilt on upstream base commit 3116c5d (UVA PLE-offload + Engram TP), lk_moe integration (README/RELEASE_NOTES/patch)
...
DeepSelect's `csrc/api.cpp` uses `<format>` (C++20) and requires GCC >= 13. On distros with an older default GCC (e.g. Rocky 9 ships GCC 11), enable a newer toolchain first, otherwise the build fails with `fatal error: format: No such file or directory`:
...
# or, cross-distro via conda: conda install -c conda-forge "gxx_linux-64>=13"
```
...
The portable patch `patches/01_lk_moe__71888f507a.patch` is generated by diffing the current tree against
the upstream base commit `71888f507a` (the local `71888f507a` commit, no network needed). **Note: the patch only contains the
...
Apply it on a clean upstream `71888f507a` checkout:
