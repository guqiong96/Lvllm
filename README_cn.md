...
```bash
2026-03-11: lvllm-v1.9.2 - FP8、AWQ4bit模型开启GPU Prefill加速不再占用额外内存[注：max_num_batched_tokens暂时有限制不超过32000], FP8模型取消TO_DTYPE运行时类型转换、KEEP暂不支持开启GPU Prefill
...
--max-num-batched-tokens 4096  
# 开启GPU预填充时，4096~32000，GPU预填充加速情况、显存大小调节，超过上下文大小无意义,当前版本限制32000
--max-num-batched-tokens 32000
```
### CPU节能
```bash
